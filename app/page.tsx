...
import { BellRing, BrainCircuit, CalendarClock, Check, CheckCircle2, ChevronRight, ClipboardCopy, Clock3, Cloud, Download, FileSpreadsheet, FileText, GraduationCap, History, KeyRound, LayoutDashboard, Link2, LoaderCircle, LogOut, Mail, Pencil, Printer, QrCode, RefreshCw, Send, Shield, Sparkles, Timer, Trash2, UploadCloud, Users, Wifi, WifiOff, X, XCircle } from "lucide-react";
...
import { csvCell, localDateTimeValue, matchesStudent } from "@/lib/client-utils";
import { validateQuizPreflight } from "@/lib/quiz-validation";
import { MathText } from "@/components/ui/math-text";
import { ThemeToggle } from "@/components/theme-toggle";
import { ScoreDistributionChart } from "@/components/eduquiz/score-chart";
...
export default function Home() {
  ...
  async function handleLogout() {
    ...
    } catch { toast.error("Chưa thể đăng xuất. Vui lòng thử lại."); return; }
    setClasses([]); setQuizzes([]); setSubmissions([]); setServices(null);
    setIsLoggedIn(false);
  ...
  return <div className="app-shell">
    <header className="topbar">
      <button className="brand" onClick={() => setSection("builder")} aria-label="Về trang tạo đề"><span className="brand-mark"><BrainCircuit size={22} /></span><span><strong>EduQuiz AI</strong><small>CLASSROOM</small></span></button>
    ...
    <div className="app-main">
      <div hidden={section !== "builder"}><QuizBuilder classes={classes} quizzesCount={quizzes.length} submissionsCount={submissions.length} teacherEmail={teacherEmail} aiConfigured={services?.aiConfigured ?? false} onSaved={(quiz) => setQuizzes((current) => [quiz, ...current.filter((item) => item.id !== quiz.id)])} /></div><div hidden={section !== "manager"}><ClassManager classes={classes} setClasses={setClasses} quizzes={quizzes} submissions={submissions} onRefresh={() => loadData(true)} /></div>
...
function QuizBuilder({ classes, quizzesCount, submissionsCount, teacherEmail, aiConfigured, onSaved }: { classes: Classroom[]; quizzesCount: number; submissionsCount: number; teacherEmail: string; aiConfigured: boolean; onSaved: (quiz: Quiz) => void }) {
  ...
  async function handleSourceFiles(fileList: FileList) {
    ...
    if (totalSize > MAX_TOTAL_BYTES) return toast.error(`Tổng dung lượng vượt quá 3 MB. Vui lòng chọn ít file hơn.`);
    const oversized = files.find((f) => f.size > 3_000_000);
    if (oversized) return toast.error(`Tệp "${oversized.name}" quá lớn. Tổng PDF/ảnh tối đa 3 MB.`);
  ...
  async function generateQuiz() {
    ...
    if (!topic.trim() && !effectiveSourceText.trim() && !(sourceMode === "file" && sourceFiles.length)) return toast.error("Vui lòng nhập chủ đề hoặc chọn tài liệu nguồn.");
  ...
  async function updateAssignment(nextClassId: string) {
    const previousClassId = assignedClassId;
    ...
    try {
      ...
      setAssignedClassId(previousClassId);
      toast.error(error instanceof Error ? error.message : "Không thể cập nhật lớp nhận bài.");
    }
  }
  return <main className="builder-page">
    <section className="overview-strip" aria-label="Tổng quan nhanh">
      <article className="overview-create"><span className="overview-icon"><Sparkles /></span><div><strong>Tạo đề AI</strong><small>Không gian giáo viên</small></div></article>
      <article><span className="overview-icon"><Users /></span><div><small>Lớp học</small><strong>{classes.length}</strong><small>Lớp đã lưu</small></div></article>
      <article><span className="overview-icon"><FileText /></span><div><small>Đề thi</small><strong>{quizzesCount}</strong><small>Đề đã tạo</small></div></article>
      <article><span className="overview-icon"><FileSpreadsheet /></span><div><small>Bài nộp</small><strong>{submissionsCount.toLocaleString("vi-VN")}</strong><small>Lượt nộp</small></div></article>
    </section>
    <div className="builder-grid">
      <aside className="config-card">
        ...
        <div className="field-block"><label className="field-label">Tài liệu nguồn</label><div className="segmented"><button className={sourceMode === "file" ? "active" : ""} onClick={() => setSourceMode("file")}>Tải file</button><button className={sourceMode === "text" ? "active" : ""} onClick={() => setSourceMode("text")}>Nhập văn bản</button></div>{sourceMode === "file" ? (<div className="dropzone-area">{fileNames.length > 0 && <div className="file-chips">{fileNames.map((name, idx) => <span key={name} className="file-chip"><span>{name}</span><button type="button" onClick={() => removeFile(idx)} aria-label={`Xóa ${name}`}><X size={12} /></button></span>)}</div>}<label ref={dropRef} className={`dropzone ${fileNames.length ? "has-file" : ""} ${isDragging ? "dragging" : ""}`} onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={(e) => { e.preventDefault(); setIsDragging(false); if (e.dataTransfer.files.length) void handleSourceFiles(e.dataTransfer.files); }}><UploadCloud size={27} /><strong>{fileNames.length ? `${fileNames.length} tệp đã chọn · kéo thêm hoặc nhấn` : "Kéo thả file vào đây"}</strong><span>{fileNames.length ? `Tổng ${fileNames.length}/${MAX_FILES} tệp · nhấn để thêm` : "hoặc nhấn để chọn từ máy"}</span><small>PDF, DOCX, TXT, JPG, PNG · tối đa 5 tệp · tổng dung lượng ≤ 3 MB</small><input type="file" multiple accept=".pdf,.docx,.txt,.jpg,.jpeg,.png" onChange={(e) => { if (e.target.files) void handleSourceFiles(e.target.files); e.target.value = ""; }} /></label></div>) : <Textarea value={sourceText} onChange={(e) => setSourceText(e.target.value)} className="min-h-36" placeholder="Dán nội dung bài học hoặc tài liệu vào đây..." />}</div>
        <details className="advanced-settings"><summary>Cài đặt đề &amp; API Key</summary><div className="advanced-content">
        ...
        <span className={`api-status ${aiConfigured ? "" : "fallback"}`}>{apiKey.trim() ? "Sẽ dùng API Key đã nhập" : aiConfigured ? "Đã cấu hình Gemini AI" : "Đang dùng bộ sinh dự phòng"}</span>
        </div></details>
        <Button className="generate-button" onClick={generateQuiz} disabled={generating}><Sparkles /> {generating ? "Đang tạo câu hỏi..." : "Tạo câu hỏi bằng AI"}</Button>
      </aside>
      <section className="workspace-card"><div className="workspace-header"><div><h2>Xem trước đề thi</h2><p>{questions.length ? `Kết quả: ${questions.length} câu hỏi` : "Bản nháp sẽ xuất hiện tại đây"}</p></div>{questions.length > 0 && <div className="workspace-actions"><Button variant="outline" aria-pressed={showAnswers} onClick={() => setShowAnswers((value) => !value)}>{showAnswers ? "Ẩn đáp án" : "Hiện đáp án"}</Button><Button variant="outline" onClick={() => void exportQuizToDocx({ id: quizId, title: topic || `${subject} ${grade}`, educationLevel, grade, subject, bloom: selectedBloom, questions, assignedClassId, teacherEmail, status: "draft", createdAt: new Date().toISOString(), maxAttempts: 3 })}><FileText /> Xuất Word (.docx)</Button><Button variant="outline" onClick={() => window.print()}><Printer /> In / Lưu PDF</Button><Button className="purple-button" onClick={publishQuiz}><QrCode /> Chia sẻ đề thi qua QR</Button></div>}</div>
        {generating ? <div className="loading-state"><span className="ai-loader"><BrainCircuit /></span><LoaderCircle className="spin" /><h3>Đang phân tích tài liệu</h3><p>Trợ lý AI đang xây dựng câu hỏi theo cấu trúc Bloom đã chọn.</p></div> : questions.length === 0 ? <div className="empty-state"><span><BrainCircuit /></span><h3>Sẵn sàng tạo câu hỏi</h3><p>Tải tài liệu hoặc nhập chủ đề ở bảng bên trái, sau đó nhấn “Tạo câu hỏi bằng AI”.</p><div className="empty-steps"><b>1</b><span>Chọn nội dung</span><ChevronRight /><b>2</b><span>Tạo đề</span><ChevronRight /><b>3</b><span>Duyệt & giao bài</span></div></div> : <div className="questions-list">{questions.map((question, index) => <QuestionCard key={question.id} question={question} index={index} showAnswers={showAnswers} onEdit={() => setEditIndex(index)} onDelete={() => setQuestions((current) => current.filter((_, i) => i !== index))} />)}</div>}
      </section>
    </div>
...
function QuestionEditor({ question, onChange, onDone }: { question: Question; onChange: (question: Question) => void; onDone: () => void }) {
  return <div className="editor-form">
    <Field label="Nội dung câu hỏi"><Textarea value={question.prompt} onChange={(e) => onChange({ ...question, prompt: e.target.value })} /></Field>
    <Field label="Mức độ Bloom"><NativeSelect className="w-full" value={question.level} onChange={(e) => onChange({ ...question, level: e.target.value as BloomLevel })}>{bloomLevels.map((item) => <NativeSelectOption key={item}>{item}</NativeSelectOption>)}</NativeSelect></Field>
    <div className="editor-options">{question.options.map((option, index) => <div key={option.id}><button className={question.correctOptionId === option.id ? "selected" : ""} onClick={() => onChange({ ...question, correctOptionId: option.id })} aria-label={`Chọn ${option.id} là đáp án đúng`}>{question.correctOptionId === option.id ? <Check /> : option.id}</button><Input value={option.text} onChange={(e) => onChange({ ...question, options: question.options.map((item, i) => i === index ? { ...item, text: e.target.value } : item) })} /></div>)}</div>
    <Field label="Lời giải chi tiết / Hướng dẫn giải" optional><Textarea value={question.explanation || ""} placeholder="Giải thích vì sao đáp án này đúng (hỗ trợ công thức Toán LaTeX)..." onChange={(e) => onChange({ ...question, explanation: e.target.value })} /></Field>
    <Button onClick={onDone}>Lưu thay đổi</Button>
  </div>;
}
...
function ClassManager({ classes, setClasses, quizzes, submissions, onRefresh }: { classes: Classroom[]; setClasses: React.Dispatch<React.SetStateAction<Classroom[]>>; quizzes: Quiz[]; submissions: Submission[]; onRefresh: () => void }) {
  ...
  const relevantSubmissions = submissions.filter((submission) => (selectedQuizId === "all" || submission.quizId === selectedQuizId) && (!selectedClass || (submission.classId ? submission.classId === selectedClass.id : normalize(submission.className) === normalize(selectedClass.name))));
  ...
  function exportCsv() {
    ...
    selectedClass.students.forEach((student) => {
      const studentSubs = relevantSubmissions.filter((item) => matchesStudent(item, student));
  ...
  return <main className="manager-page"><section className="page-heading manager-heading"><div><span className="eyebrow"><LayoutDashboard size={14} /> QUẢN LÝ LỚP HỌC</span><h1>Quản lý & Kiểm soát học sinh làm bài</h1><p>Theo dõi tiến độ, đối chiếu bằng mã học sinh và gửi nhắc chưa nộp.</p></div><Button variant="outline" onClick={onRefresh}><RefreshCw /> Đồng bộ dữ liệu</Button></section>
    ...
    <Tabs value={tab} onValueChange={setTab} className="manager-tabs"><TabsList variant="line" className="manager-tablist"><TabsTrigger value="history"><History /> Lịch sử bài làm <span>{submissions.length}</span></TabsTrigger><TabsTrigger value="monitor"><Users /> Kiểm soát làm / chưa làm <em>Mới</em></TabsTrigger><TabsTrigger value="classes"><GraduationCap /> Danh sách lớp học <span>{classes.length}</span></TabsTrigger></TabsList>
      <TabsContent value="history">
        <div className="stat-grid five"><Stat label="Lượt nộp bài" value={submissions.length} suffix="lượt" tone="purple" icon={<Send />} /><Stat label="Điểm trung bình" value={submissions.length ? (submissions.reduce((sum, item) => sum + item.score, 0) / submissions.length).toFixed(1) : "0"} suffix="/ 10" tone="green" icon={<FileSpreadsheet />} /><Stat label="Điểm cao nhất" value={submissions.length ? Math.max(...submissions.map((item) => item.score)) : 0} suffix="đ" tone="amber" icon={<CheckCircle2 />} /><Stat label="Điểm thấp nhất" value={submissions.length ? Math.min(...submissions.map((item) => item.score)) : 0} suffix="đ" tone="rose" icon={<XCircle />} /><Stat label="Tỷ lệ hoàn thành" value={progress} suffix="%" tone="blue" icon={<Users />} /></div>
        <ScoreDistributionChart submissions={submissions} />
        <SubmissionTable rows={submissions} quizzes={quizzes} />
      </TabsContent>
      <TabsContent value="monitor"><div className="monitor-toolbar"><div className="toolbar-filters"><Field label="Lớp"><NativeSelect value={selectedClass?.id || ""} onChange={(event) => { setSelectedClassId(event.target.value); setSelectedQuizId("all"); }}>{classes.map((item) => <NativeSelectOption key={item.id} value={item.id}>{item.name} · {item.code}</NativeSelectOption>)}</NativeSelect></Field><Field label="Chọn bài tập"><NativeSelect value={selectedQuizId} onChange={(event) => setSelectedQuizId(event.target.value)}><NativeSelectOption value="all">Tất cả bài tập</NativeSelectOption>{relevantQuizzes.map((item) => <NativeSelectOption key={item.id} value={item.id}>{item.title}</NativeSelectOption>)}</NativeSelect></Field></div><div className="toolbar-actions"><Button variant="outline" onClick={sendReminders} disabled={reminding || selectedQuizId === "all"}><BellRing /> {reminding ? "Đang gửi..." : "Nhắc Email / Zalo"}</Button><Button className="excel-button" onClick={exportCsv}><FileSpreadsheet /> Xuất Bảng Điểm</Button></div></div>
        <ScoreDistributionChart submissions={relevantSubmissions} />
...
function LoginPage({ googleConfigured, onLogin }: { googleConfigured: boolean; onLogin: (email: string, accessCode: string) => Promise<string | null> }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    } catch { toast.error("Không thể kết nối. Vui lòng thử đăng nhập lại."); }
  ...
  return (
    <div className="login-bg">
      ...
      <div className="login-card">
        <div className="login-logo"><BrainCircuit size={36} /><div><strong>EduQuiz AI</strong><small>CLASSROOM</small></div></div>
